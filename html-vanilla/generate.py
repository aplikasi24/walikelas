...
<head>
    ...
    <title>{page_title} | Manajemen Wali Kelas</title>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto w-full">
        <div class="p-6 md:p-10 w-full max-w-7xl mx-auto space-y-6 animate-fade-in">
            <header class="mb-8 border-b border-gray-200 pb-4">
                ...
                <p class="text-gray-500 mt-1">{page_desc}</p>
            </header>
            ...
            <div class="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 min-h-[400px] flex flex-col items-center justify-center text-center">
                <div class="w-24 h-24 bg-blue-50 rounded-full flex items-center justify-center mb-6">
                    <i class="fas {page_icon} text-4xl text-blue-400"></i>
                </div>
                <h2 class="text-2xl font-semibold text-gray-700 mb-2">Halaman {page_title}</h2>
                <p class="text-gray-500 max-w-md">
                    Ini adalah kerangka halaman untuk fitur {page_title}. Anda dapat mengganti bagian ini dengan tabel data, form input, atau laporan grafis yang sesuai.
                </p>
...
nav_template_active = """            <a href="{href}" class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 bg-blue-800 border-l-4 border-yellow-400 text-yellow-400 shadow-md group">
                <i class="fas {icon} w-5 text-center text-yellow-400"></i>
                <span class="font-medium text-sm">{name}</span>
            </a>"""

nav_template_inactive = """            <a href="{href}" class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 text-blue-100 hover:bg-blue-800 hover:text-white border-l-4 border-transparent group">
                <i class="fas {icon} w-5 text-center text-blue-300 group-hover:text-white transition-colors"></i>
                <span class="font-medium text-sm">{name}</span>
            </a>"""

# Extra content for Dashboard (index.html) to keep it rich
dashboard_extra = """
...
output_dir = "C:\\Users\\LENOVO\\.gemini\\antigravity\\scratch\\manajemen-wali-kelas\\html-vanilla"
...
for page in menus:
    ...
    for item in menus:
        if item["file"] == page["file"]:
            nav_html_lines.append(nav_template_active.format(href=item["file"], icon=item["icon"], name=item["name"]))
        else:
            nav_html_lines.append(nav_template_inactive.format(href=item["file"], icon=item["icon"], name=item["name"]))
    
    nav_items_str = "\n".join(nav_html_lines)
    
    extra = dashboard_extra if page["file"] == "index.html" else ""
    
    html_content = template.format(
        page_title=page["title"],
        page_desc=page["desc"],
        page_icon=page["icon"],
        nav_items=nav_items_str,
        extra_content=extra
    )
    
    with open(os.path.join(output_dir, page["file"]), 'w', encoding='utf-8') as f:
